 ...
 * then modified for part B of hw9 to calculate gas station sales
 */
 ...
 * @retval None
 */
void freememory(int xr, int yr, int ***y, int ***z)
{
...
void matrixprint(int xr, int xc, int *x, int yr, int yc, int **y, int zr, int zc, int **z)
{
	...
	//edited printrow function due to matrix x pointer issue(* not **) and 1 row   
	if (r < xr) // We have data for this row
        {
        	for (int c = 0; c < xc; c++){
            		printf("%-*d ", display_width, x[c]);
        	}
    	}
   	else // Array doesn't have enough rows, just pad with white space
    	{
...
/**
 * @brief will get a days sales and put into matrix
 * @retval None
 */
void getsales(int yr, int yc, int **y){
	
	printf("Enter sales in the format: regular#special#premium:\n\n");
	
	for (int i = 1; i < 4; i++){ //loops to count 3 days of sales
		printf("Enter total gallons of gas sales on Day %d: ",i);
		scanf("%d#%d#%d",&y[0][i-1],&y[1][i-1],&y[2][i-1]);
 ...
 * 	   Many parts are commented out due to being unnecessary in "hw9 Part B"
 * @retval 0 if the program terminates without errors
 */
int main()
{

    // initialize dimension variables based on required input format
    int xr = 1, xc = 3, yr = 3, yc = 3;
    ...
    //fillmatrix(yr, yc, y); // Just to initialize - will be replaced
    fillmatrix(xr, yc, z); // Just to initialize - will be replaced

    getsales(yr, yc, y); //will gets sales data and put in matrix "y"
	
    multiplymatrix(xr, xc, prices, yr, yc, y, xr, yc, z); // will multiply the 2 matrices together

    matrixprint(xr, xc, prices, yr, yc, y, xr, yc, z); // will print 2 beginning matrices and matrix product of the two
    
    //deallocate memory from all the matrices
    freememory(xr, yr, &y, &z);
